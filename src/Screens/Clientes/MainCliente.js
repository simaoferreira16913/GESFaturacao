...
export default function MainCliente({navigation}) {
  ...
  const data = clientes.map(item => {
    ...
    return [ item[2],item[3],  botoes];
  });

  const handleRemove = (id) => {
    console.log(id)
    deletecliente(id).then((res)=>{
      console.log(res);
    });

    setClientes(clientes.filter(item => item[0] !== id));
