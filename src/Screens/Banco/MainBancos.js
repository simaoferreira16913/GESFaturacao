...
export default function MainBancos({navigation}) {
  LogBox.ignoreLogs(['Warning: ...']); // Ignore log notification by message
  LogBox.ignoreAllLogs();//Ignore all log notifications
  
  const {deleteBanco} = useContext(AuthContext);
  const { getBancos } = useContext(AuthContext);
  ...
  const removerConfirmar = (id) =>{
    console.log(id)
    deleteBanco(id).then((res)=>{
      console.log(res);
    });

    setBancos(bancos.filter(item => item[0] !== id));
    ToastAndroid.show("Banco Eliminado",ToastAndroid.SHORT);
  }
  ...
  return (
    ...
    <View style={styles.container}>
     ...
     <View > 
      ...
      <Table style={{width: '100%', height: '100%', marginLeft: '35%'}}>
